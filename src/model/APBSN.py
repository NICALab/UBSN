...
class APBSN(nn.Module):
    ...
    def __init__(self, bsn, pd, dilation, width, 
        ...
        # define network
        if bsn == 'UBSN':
            self.bsn = self.bsn = UBSN(pd, depth=3, in_ch=3, blind_conv_channels=128, one_by_one_channels=[48, 24, 8, 1], bs_size=[1, 1], deep_0=5, deep_1=3, deep_2=0)
        else:
